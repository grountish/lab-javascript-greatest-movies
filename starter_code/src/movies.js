...
function orderByYear(movies) {
    let newArray = [];
    newArray = JSON.parse(JSON.stringify(movies));
...
function howManyMovies(movies) {
    ...
    var len = newArray.filter(x => x.director === "Steven Spielberg" && x.genre.indexOf('Drama') != -1);
    return len.length;
}
...
function orderAlphabetically(movies) {
    ...
    return twentyMovies.map(x => x.title);
}
...
function ratesAverage(array) {
    ...
    let rates = newArray.map(x => {
        return x.rate
    });
    rates = rates.filter(x => x > 0)
    let avg = rates.reduce((a, b) => a + b)
...
function dramaMoviesRate(array) {
    let newArray = [...array]
    var len = newArray.filter(x => x.genre.indexOf('Drama') != -1)
    let dramaMovies = ratesAverage(len);
    return dramaMovies;
